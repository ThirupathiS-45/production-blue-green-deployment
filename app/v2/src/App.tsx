...
const CONFIG = {
  version: '2.0.0',
  environment: 'GREEN',
  envLabel: 'Release Candidate',
  themeClass: 'green-theme',
  buildId: '#1,248',
  deployedAt: 'Today 22:18 IST',
  port: '8082',
}
...
export default function App() {
  ...
  const [cpu, setCpu] = useState(31.8)
  const [mem, setMem] = useState(54.2)
  const [rps, setRps] = useState(1247)

  // Live metric updates every 2 s — v2 shows better performance
  useEffect(() => {
    const t = setInterval(() => {
      setTime(new Date())
      setCpu(p => +Math.max(18, Math.min(62, p + (Math.random() - 0.5) * 6)).toFixed(1))
      setMem(p => +Math.max(40, Math.min(72, p + (Math.random() - 0.5) * 3)).toFixed(1))
      setRps(p => Math.max(900, Math.min(1600, p + Math.round((Math.random() - 0.5) * 90))))
    }, 2000)
    return () => clearInterval(t)
  }, [])

  // Animated stats — v2 has better numbers
  const uptime   = useCounter(9999)
  const deploys  = useCounter(248)
  const success  = useCounter(99)
  const respTime = useCounter(89)

  const health: HealthItem[] = [
    { name: 'Application Server (v2)',    status: 'healthy', latency: '8ms'  },
    { name: 'PostgreSQL Database',        status: 'healthy', latency: '3ms'  },
    { name: 'Redis Cache (Clustered)',    status: 'healthy', latency: '1ms'  },
    { name: 'RabbitMQ Queue',            status: 'healthy', latency: '6ms'  },
    { name: 'CDN / Static Assets',       status: 'healthy', latency: '18ms' },
    { name: 'Auth Service (JWT + OAuth)', status: 'healthy', latency: '4ms'  },
  ]
  ...
  const features: Feature[] = [
    ...
    { name: 'Advanced Analytics',       status: 'enabled', description: 'Real-time insights dashboard',  isNew: true },
    { name: 'AI Deployment Prediction', status: 'beta',    description: 'ML-powered risk scoring',        isNew: true },
  ]

  const history: Deployment[] = [
    { version: 'v2.0.0', time: 'Just now',    status: 'success',  duration: '1m 28s' },
  ...
  return (
    <div className={`app ${CONFIG.themeClass}`}>
      ...
      <div className="stats-bar">
        <StatCard icon="⚡" label="Uptime"         value={`${(uptime/100).toFixed(2)}%`} sub="30-day SLA"    />
        <StatCard icon="⏱️" label="Response Time"  value={`${respTime}ms`}               sub="p95 latency ↓28%" />
        ...
        <div className="col">
          ...
          <div className="card">
            ...
            <div className="perf-list">
              <ProgressBar label="CPU Usage"    value={cpu} color="#22c55e"   />
              <ProgressBar label="Memory Usage" value={mem} color="#a855f7"   />
              <ProgressBar label="Disk Usage"   value={29}  color="#f59e0b"   />
